...
public:
    int findClosest(vector<string>& words, string word1, string word2) {
        ...
        for(int i = 0  ; i < n; i ++)
        {
            if(words[i].compare(word1) == 0)  map[word1].push_back(i);
            else if (words[i].compare(word2) == 0)  map[word2].push_back(i);
        }

        for(auto i : map[word1])
        {
            for(auto j : map[word2]) distance = (abs(i-j) < distance) ? abs(i-j) : distance ;
        }
